...
Route::get('pp', function () {
	...
	// Read the JSON file
	$json = file_get_contents('../routes/ceramica.json', FILE_USE_INCLUDE_PATH);
	// Decode the JSON file
	$json_data = json_decode($json, true);

	/* foreach ($json_data as $pr) {
		$product = new Product();
		$product->supplier_id = $pr['supplier_id'];
		...
		$product->selected_categories_from_root = '';
		...
		$product->money = 1;
		$product->status_id = 1;
		$product->priority_id = 10;
		$product->attributes = $pr['attributes'];
		$product->stock = $pr['stock'];
		$product->critical_stock = $pr['critical_stock'];
		$product->mts_by_unity = ($pr['mts_by_unity']) ? $pr['mts_by_unity'] : null;
		$product->see_price_on_the_web = 1;
		$product->isCHP = $pr['isCHP'];
		$product->published_here = 1;
